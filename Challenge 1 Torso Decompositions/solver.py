...
CONFIG = {
    "general": {
        ...
        "restart_stagnation_trigger": 150, "restart_fraction": 0.3
    },
        ...
        for gen in pbar:
                ...
                while len(offspring_sols) < len(pop):
                    ...
                    if random.random() < mutation_rate:
                        if random.random() < 0.5:
                            a, b = sorted(random.sample(range(n), 2))
                            child_perm[a:b+1] = child_perm[a:b+1][::-1]
                        else:
            ...
            if stagnation_counter > config['restart_stagnation_trigger']:
                ...
                for i in range(num_islands):
                    ...
                    islands[i].sort(key=lambda p: (p['score'][0], -p['score'][1])) # Sort worst to best
                    for j in range(num_to_replace):
                        np.random.shuffle(base_perm)
                        t = np.random.randint(int(n * 0.1), int(n * 0.5))
                        islands[i][j]['solution'] = np.append(base_perm.copy(), t)
                for i in range(num_islands):
                    num_to_replace = int(len(islands[i]) * config['restart_fraction'])
                    sols_to_eval = [islands[i][j]['solution'] for j in range(num_to_replace)]
                    results = pool.map(eval_wrapper, sols_to_eval)
                    res_dict = {tuple(sol): score for sol, score in results}
                    for j in range(num_to_replace):
                        islands[i][j]['score'] = res_dict.get(tuple(islands[i][j]['solution']))
                ...
                if migrants:
                    for island in islands:
                        for j in range(len(migrants)):
                            if j < len(island): island[-(j+1)]['solution'] = random.choice(migrants)
