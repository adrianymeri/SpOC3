...
# ==============================================================================
class TorsoProblem:
    def __init__(self, problem_id: str, initialize_cython: bool = False):
        self.problem_id = problem_id
        if not hasattr(self, 'n_nodes'): # Avoid reloading in worker
...
def evolve_island(args: Tuple[np.ndarray, int]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    pop.set_x(initial_vectors) # Seed with the provided population
...
if __name__ == "__main__":
    ...
    total_pop_size = num_islands * pop_size_per_island

    print(f"🧬 Initializing {num_islands} island populations of size {pop_size_per_island}...")
    # Create one large initial population
    ...
    with ProcessPoolExecutor(max_workers=num_islands, initializer=init_worker, initargs=(problem_id,)) as executor:
        for evo_step in range(n_evolutions):
            ...
            # --- Gather All Solutions for Migration ---
            ...
            # Distribute the best individuals among all islands for the next generation
            for i in range(num_islands):
                # Get the current island's population
                current_island_x = results[i][0]
                # Replace the worst individuals with the best from the global pool
                num_to_replace = min(config['migration_size'], len(best_individuals_x), len(current_island_x))
                migrants = random.sample(list(best_individuals_x), num_to_replace)
                current_island_x[:num_to_replace] = migrants
