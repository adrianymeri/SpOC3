#!/usr/bin/env python3
"""
hc5_operators.py — Pareto archive HC + min-degree warm start + 15 operators.

Difference from hc4_warm_start
------------------------------
hc3 used a single mutator (random pairwise swap, 90%) plus a 10% chance
of t-resample.  hc4 keeps the same archive + warm-start scaffolding but
swaps in a richer operator set:

    Permutation operators (act on perm only):
      - swap                random pairwise swap
      - adjacent_swap       swap two adjacent positions
      - insert              pop one position, insert elsewhere
      - reverse             2-opt: reverse a random segment perm[i..j]
      - 3opt                3-opt: simultaneously reverse two adjacent
                            non-overlapping segments perm[i..j) and
                            perm[j..k) — a genuinely 3-opt move not
                            reachable from a single reverse or block_move
      - block_move          cut a small block, paste elsewhere (or-opt)
      - long_block_move     same but with up to ~20% of n
      - bottleneck_to_head  move the max-width vertex (or one of its
                            torso successors) into the head [0, t)
      - swap_head_tail      swap a head vertex with a torso vertex
      - 2opt_torso          reverse a segment that lies inside the torso

    Threshold operators (act on t):
      - t_shift             ±5% of n
      - t_random            uniform random t

Selection: weighted by recent success rate (accepts/(attempts+1)) with
+1 priors so every operator stays alive.

Goal: isolate the impact of operator diversity on top of hc3's archive +
warm-start.
"""
...
# min-fill-reinsert operator wrapper (operators are module-level).
_ADJ_BITS: List[int] | None = None


# ---------------------------------------------------------------------------
# Operators.  Each takes (perm, n, t, bn_idx, bn_mask) and returns
# (new_perm, new_t).  perm is mutated *in place* and the caller is
# responsible for undoing if rejected; for cleanliness we make a copy here.
# ---------------------------------------------------------------------------
...
def op_reverse(perm, n, t, bn_idx, bn_mask):
    """Classical 2-opt for permutations: reverse perm[i..j]."""
    out = perm[:]
...
def op_3opt(perm, n, t, bn_idx, bn_mask):
    """Classical 3-opt 'double-reverse' move: pick i < j < k and
    simultaneously reverse perm[i:j] AND perm[j:k].  This is a
    genuinely 3-opt neighbour (not reachable by any single 2-opt or
    block_move from the current perm) and gives the search a third
    structurally distinct mutation primitive alongside 2-opt and
    block-move/or-opt.  Indices are sampled with j − i ≥ 2 and
    k − j ≥ 2 so both segments contain a non-trivial reversal."""
    if n < 6:
        return op_reverse(perm, n, t, bn_idx, bn_mask)
    out = perm[:]
    # Pick i < j < k with both inner segments of length >= 2.
...
OPERATORS: List[Tuple[str, Callable]] = [
    ...
    ("insert",            op_insert),
    ("reverse",           op_reverse),         # 2-opt (segment reversal)
    ("3opt",              op_3opt),            # 3-opt (double-segment reversal)
    ...
    ("or_opt",            op_or_opt_w),           # Or-opt (segment-shift, no reverse)
...
def run(
    ...
    md, ws_label = build_warm_start(n, adj_bits, rng=md_rng, method=warm_start)
    ...
    while time.time() < deadline:
        ...
        # Adaptive operator selection (UCB-ish: accepts+1 / attempts+1).
        weights = [
    ...
    out_path = submission_path(here, problem, "hc5")
