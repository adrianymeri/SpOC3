...
def main():
    ...
    parser.add_argument("--focus-sizes", type=str, default="",
                        help="comma list of torso sizes to concentrate breeding on (from cap_submit)")
    parser.add_argument("--focus-mass", type=float, default=0.9,
    ...
    if args.focus_sizes:
        ...
        w = torch.full((N, ), (1.0 - args.focus_mass) / N, dtype=torch.float32)
        per = args.focus_mass / max(len(sizes), 1)
        hw = max(args.focus_halfwidth, 0)
        for s in sizes:
            i = min(max(N - s, 0), N - 1)
            lo, hi = max(i - hw, 0), min(i + hw + 1, N)
            w[lo:hi] += per / (hi - lo)
        elite_range = (w / w.sum()).cuda()
        print(f"cap-focus ON: {len(sizes)} target sizes, {args.focus_mass:.0%} of breeding mass in +-{hw} windows", flush=True)
    else:
        elite_range = elite_range / B
